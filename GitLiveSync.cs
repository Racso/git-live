...
namespace GitLive
{
    public class GitLiveSync
    {
        ...
        public class SyncOptions
        {
            ...
            public List<string> FileSelectionRules { get; set; } = new List<string>();
        ...
        public SyncResult Sync(SyncOptions options)
        {
            ...
            try
            {
                ...
                // If LIVE/main isn't present that's ok for an empty remote — we'll create the branch when pushing.
                if (liveMainTip == null)
                    options.Logger.VeryVerbose("LIVE/main not found; proceeding as empty remote.");
                ...
                var createdTagToCommit = PublishTags(gp, repoTagsWithData, startIndex, tmpBranch, ref liveMainTip, 
                    ref baseForRange, options.FileSelectionRules, options.Mode, options.Logger);
            ...
            out string tempDir, out GitRunner publisherGit, ConsoleLogger logger)
        {
            ...
            // If the LIVE remote is a local path, try to set receive.denyCurrentBranch to ignore
            // so we can push into a non-bare repository used by tests.
            try
            {
                publisherGit.TryRun($"-C {EscapeArg(liveRemoteUrl)} config receive.denyCurrentBranch ignore");
            }
            catch
            {
            }
            ...
            var liveRemoteProbe = publisherGit.TryRun("ls-remote LIVE") ?? "";
             // If the LIVE remote has no refs (completely empty repository) ls-remote will return empty.
             // Treat an empty remote as valid — we'll create the initial history when publishing.
             if (string.IsNullOrWhiteSpace(liveRemoteProbe))
             {
                 logger.VeryVerbose("LIVE remote appears empty (no refs). Proceeding to create history on publish.");
             }
        ...
        private string CreateTemporaryBranch(GitRunner gp, string? liveMainTip, SyncMode mode, ConsoleLogger logger)
        {
            ...
            // If we have a liveMainTip, set the tmp branch to that commit. Otherwise leave it uninitialized;
            // PublishTags will create commits and update the tmp branch as we go.
            if (mode != SyncMode.Nuke && !string.IsNullOrEmpty(liveMainTip))
        ...
        private Dictionary<string, string>? PublishTags(GitRunner gp, 
            ...
            List<string> fileSelectionRules, SyncMode mode, ConsoleLogger logger)
        {
            ...
            for (int i = startIndex; i < repoTagsWithData.Count; i++)
            {
                ...
                // Apply file selection rules if provided
                if (fileSelectionRules.Count > 0)
                {
                    var selector = new FileSelector();
                    foreach (var rule in fileSelectionRules)
                    {
                        selector.AddRule(rule);
                    }
                    treeId = selector.FilterTree(gp, treeId);
                }
                ...
                // If there is no current parent (empty LIVE repo) we should create the initial commit without -p.
                bool makeRootCommit = (mode == SyncMode.Nuke && i == startIndex) || string.IsNullOrEmpty(currentParent);
                string commitTreeArgs = makeRootCommit
